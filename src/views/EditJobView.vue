<template>
  <section class="bg-green-50">
    <div class="container m-auto max-w-2xl py-24">
      ...
      >
        <form @submit.prevent="handleUpdateJobFormSubmit">
...
onMounted(async () => {
  try {
    ...
    jobs.value = response
    form.title = jobs.value.title
    form.type = jobs.value.type
    form.description = jobs.value.description
    form.location = jobs.value.location
    form.salary = jobs.value.salary
    form.company.name = jobs.value.company.name
    form.company.description = jobs.value.company.description
    form.company.contactEmail = jobs.value.company.contactEmail
    form.company.contactPhone = jobs.value.company.contactPhone
  } catch (error) {
    console.log('Error is caught: ', error)
  }
})

const handleUpdateJobFormSubmit = async () => {
  const newJob = {
    title: form.title,
    type: form.type,
